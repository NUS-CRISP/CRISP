...
export const getAuthorizedTeamDataByCourse = async (
  ...
  if (
    ...
    if (!course.gitHubOrgName && (!course.gitHubRepoLinks || course.gitHubRepoLinks.length === 0)) {
      throw new NotFoundError('Course GitHub organization or repository links not found');
    }
