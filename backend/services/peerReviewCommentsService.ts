...
const UNAUTHORIZED_TO_VIEW_COMMENTS = 'You are not authorized to see comments for this assignment';
const UNAUTHORIZED_TO_ADD_COMMENTS = 'You are not authorized to add comments for this assignment';
const UNAUTHORIZED_TO_UPDATE_COMMENTS = 'You are not authorized to update comments for this assignment';
const UNAUTHORIZED_TO_DELETE_COMMENTS = 'You are not authorized to delete comments for this assignment';

export const getPeerReviewCommentsByAssignmentId = async (userId: string, userCourseRole: string, assignmentId: string) => {
  ...
  if (!revieweeTeam) throw new NotFoundError('Reviewee team not found for this assignment');
  ...
  if (userCourseRole === CourseRole.Student) {
    ...
    if (isReviewee) return PeerReviewCommentModel.find({ peerReviewAssignmentId: assignmentId }).populate('author', 'name');
    if (isReviewer) {
  ...
  if (userCourseRole === CourseRole.TA) {
    ...
    if (isSupervisingTA) return PeerReviewCommentModel.find({ peerReviewAssignmentId: assignmentId }).populate('author', 'name');
    if (isReviewer) {
      return PeerReviewCommentModel.find({
        peerReviewAssignmentId: assignmentId,
        author: userId,
  ...
  if (userCourseRole === CourseRole.Faculty) {
    return PeerReviewCommentModel.find({ peerReviewAssignmentId: assignmentId }).populate('author', 'name');
  }
  
  throw new MissingAuthorizationError(UNAUTHORIZED_TO_VIEW_COMMENTS);
}

export const addPeerReviewCommentByAssignmentId = async (userId: string, userCourseRole: string, assignmentId: string, commentData: any) => {
  const assignment = await PeerReviewAssignmentModel.findById(assignmentId);
  if (!assignment) throw new NotFoundError(ASSIGNMENT_NOT_FOUND);
  
  // If student is not the reviewer user nor part of the reviewer team, they cannot comment
  if (userCourseRole === CourseRole.Student && assignment.reviewerUser?.toString() !== userId) {
    ...
    const isReviewerTeam = reviewerTeam && reviewerTeam.members?.map(String).includes(userId);
  ...
  if (userCourseRole === CourseRole.TA && assignment.reviewerUser?.toString() !== userId){
    const isReviewerUser = assignment.reviewerUser?.toString() === userId;
    const revieweeTeam = await TeamModel.findById(assignment.reviewee);
    const isSupervisingTA = revieweeTeam && revieweeTeam.TA!.toString() === userId;
...
export const updatePeerReviewCommentById = async (userId: string, commentId: string, updatedComment: string) => {
...
export const deletePeerReviewCommentById = async (userId: string, userCourseRole: string, commentId: string) => {
  const comment = await PeerReviewCommentModel.findById(commentId);
  if (!comment) throw new NotFoundError(COMMENT_NOT_FOUND);
  
  const assignment = await PeerReviewAssignmentModel.findById(comment.peerReviewAssignmentId);
  if (!assignment) {
    throw new NotFoundError('Peer review assignment not found for this comment');
  }
  ...
  if (userCourseRole === CourseRole.Student && userId === comment.author.toString()) {
    const deletedComment = await PeerReviewCommentModel.deleteOne({ _id: commentId });
