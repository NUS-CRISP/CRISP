...
export const getPeerReviewAssignmentsByPeerReviewId = async (userId: string, userCourseRole: string, peerReviewId: string) => {
  ...
  if (userCourseRole === CourseRole.Student) {
    const studentAssignments = await PeerReviewAssignmentModel.find({ peerReviewId, reviewerUser: userId });
...
export const getPeerReviewAssignmentsByTeamId = async (userCourseRole: string, teamId: string) => {
  // Only course coordinators and TAs can view assignments by team
  if (userCourseRole !== CourseRole.Faculty && userCourseRole !== CourseRole.TA) {
    throw new MissingAuthorizationError('Only course coordinators and TAs can view peer review assignments by team');
  }
  
  const assignments = await PeerReviewAssignmentModel.find({ reviewee: teamId });
  if (!assignments) {
...
export const getPeerReviewAssignmentById = async (userCourseRole: string, userId: string, assignmentId: string) => {
  ...
  if (userCourseRole === CourseRole.Student) {
    ...
    if (reviewerTeam && reviewerTeam.members?.map(String).includes(userId)) return assignment;
    
    // Check if student is part of the reviewee team
    const revieweeTeam = await TeamModel.findById(assignment.reviewee);
    if (revieweeTeam && revieweeTeam.members?.map(String).includes(userId)) return assignment;
  }
  ...
  throw new MissingAuthorizationError('You are not authorized to view this assignment');
}

export const assignPeerReviewById = async (peerReviewId: string, assignmentData: any) => {
