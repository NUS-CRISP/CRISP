...
export const getPeerReviewInfoById = async (
  ...
  const { teamIds, filterByTA } = await getScopedTeamIds(userId, userCourseRole, teamSetId);
  const scopedTeams = await getScopedTeams(teamSetId, teamIds, filterByTA);
  if (scopedTeams.length === 0) return emptyPeerReviewInfo(peerReviewId, reviewerType);
  
  const prTeamIds = scopedTeams.map(t => t.id);
  const teamDataById = await getTeamDataById(courseId, scopedTeams.map(t => t.number));
  ...
  const { memberAssignmentsByUserId, teamAssignmentsByTeamId } = buildAssignmentMaps(reviewerType, scopedTeams, prAssignments);
...
export const assignPeerReviews = async (
  ...
  const getTeamRepo = (teamId: string) => {
    ...
    return teamIdToRepoMap.get(num.toString()) || { repoUrl: '', repoName: '', gitHubOrgName: '' };
  }
  
  // Build Reviewer Pools
  const { studentReviewers, studentHomeTeam, teamReviewers, taReviewers, taHomeTeams } = buildReviewerPools(prTeams, prTeamIds, taAssignmentsEnabled);
  
  // Get Eligible Reviewwes
  const { studentToEligibleRevieweesMap, teamToEligibleRevieweesMap, taToEligibleRevieweesMap } =
...
export const addManualAssignment = async (
  ...
  const reviewee = await TeamModel.findOne({
    _id: revieweeId,
    teamSet: teamSetId,
  }).select('_id number members TA').lean();
  if (!reviewee) {
    throw new NotFoundError('Reviewee team not found in the peer review team set');
  }
  ...
  if (reviewerType === 'Individual') {
    ...
    if (!reviewerTeam) {
      throw new NotFoundError('Reviewer not found as a member of any team in the peer review team set');
    }
    ...
    if (!reviewerTeam) {
      throw new NotFoundError('Reviewer team not found in the peer review team set');
    }
  ...
  if (reviewerType === 'Individual') {
    const currentAssignmentsCount = await PeerReviewAssignmentModel.countDocuments({
      peerReviewId,
      studentReviewers: reviewerId,
    });
    if (currentAssignmentsCount >= maxReviewsPerReviewer) {
      throw new BadRequestError('Reviewer has reached the maximum number of assigned reviews');
    }
  } else {
    const currentAssignmentsCount = await PeerReviewAssignmentModel.countDocuments({
      peerReviewId,
    ...
    if (currentAssignmentsCount >= maxReviewsPerReviewer) {
      throw new BadRequestError('Reviewer team has reached the maximum number of assigned reviews');
    }
  }
  ...
  try {
    await session.withTransaction(async () => {
      const add = reviewerType === 'Individual'
...
export const removeManualAssignment = async (
  ...
  const existingAssignment = await PeerReviewAssignmentModel.findOne({
    ...
    ...(reviewerType === 'Individual'
        ? { studentReviewers: reviewerId }
        : { teamReviewers: reviewerId }),
  }).select('_id studentReviewers teamReviewers taReviewers').lean();
  if (!existingAssignment) {
    return; // Nothing to remove
  }
  
  const pull = reviewerType === 'Individual'
...
const emptyPeerReviewInfo = (peerReviewId: string, reviewerType: "Individual" | "Team") => {
  return {
    _id: peerReviewId,
    teams: [],
...
const getScopedTeamIds = async (userId: string, userCourseRole: string, teamSetId: string) => {
...
const getScopedTeams = async (teamSetId: string, teamIds: string[], filterByTA?: string): Promise<NormalizedTeam[]> => {
...
const getAssignmentsForTeams = async (
  ...
  if (reviewerType === 'Individual') {
    ...
    const orConditions = [
      { studentReviewers: { $in: allMemberIds.map(id => new Types.ObjectId(id)) } },
    ...
    for (const reviewer of a.studentReviewers) {
      if (taIdsWantedSet.has(reviewer._id)) {
        const val = assignmentsForTAs[reviewer._id] ?? {
          taName: usersById.get(reviewer._id) ?? 'Unknown',
          assignedReviews: [] as PeerReviewAssignment[],
        }
...
const buildReviewerPools = (prTeams: NormalizedTeam[], prTeamIds: string[], taAssignmentsEnabled: boolean) => {
  ...
  const taReviewers = Array.from(new Set(prTeams.map(t => t.taId).filter((x): x is string => Boolean(x))))
  ...
  return { studentReviewers, studentHomeTeam, teamReviewers, taReviewers, taHomeTeams };
}
...
const buildEligibleRevieweeMaps = (
  ...
  if (reviewerType === 'Individual') {
    for (const studentId of studentReviewers) {
      ...
      const eligibleReviewees = makeEligibleReviewees(teamIdToTAMap, prTeamIds, allowSameTA, homeTeamId);
  ...
  if (reviewerType === 'Team') {
    for (const teamId of teamReviewers) {
      const eligibleReviewees = makeEligibleReviewees(teamIdToTAMap, prTeamIds, allowSameTA, teamId);
      if (eligibleReviewees.length < reviewsPerReviewer) {
        throw new BadRequestError(
  ...
  if (taAssignmentsEnabled) {
    for (const taId of taReviewers) {
      ...
      const eligibleReviewees = prTeamIds.filter(tid => !supervisedTeams.has(tid)); // cannot review own team
      if (eligibleReviewees.length < reviewsPerReviewer) {
        throw new BadRequestError(
  ...
  return { studentToEligibleRevieweesMap, teamToEligibleRevieweesMap, taToEligibleRevieweesMap };
}
...
const performAssignments = (
  ...
  if (reviewerType === 'Individual') {
    randomAndEqualAssign(studentReviewers, reviewsPerReviewer, studentToEligibleRevieweesMap, assignedStudentsForTeam, "student");
  } else {
    randomAndEqualAssign(teamReviewers, reviewsPerReviewer, teamToEligibleRevieweesMap, assignedTeamsForTeam, "team");
  }
  
  if (taAssignmentsEnabled && taReviewers.length > 0) {
    randomAndEqualAssign(taReviewers, reviewsPerReviewer, taToEligibleRevieweesMap, assignedTAsForTeam, "TA");
  }
...
const updateDBAssignments = async (
  ...
  try {
    await session.withTransaction(async () => {
      await PeerReviewAssignmentModel.deleteMany({ peerReviewId }, { session });
      const newAssignments = prTeamIds.map(revieweeId => {
        const { repoUrl, repoName, gitHubOrgName } = getTeamRepo(revieweeId);
        const studentSet = assignedStudentsForTeam.get(revieweeId)!;
        const teamSet = assignedTeamsForTeam.get(revieweeId)!;
        const taSet = assignedTAsForTeam.get(revieweeId)!;
        
        return {
          peerReviewId,
          repoName,
          repoUrl,
          studentReviewers: reviewerType === 'Individual' ? Array.from(studentSet).map(id => new Types.ObjectId(id)) : [],
          teamReviewers: reviewerType === 'Team' ? Array.from(teamSet) : [],
          taReviewers: taAssignmentsEnabled ? Array.from(taSet) : [],
          reviewee: new Types.ObjectId(revieweeId),
          assignedBy: new Types.ObjectId(userId),
          assignedAt: new Date(),
          status: 'Pending' as const,
      ...
      if (newAssignments.length > 0) {
        await PeerReviewAssignmentModel.insertMany(newAssignments, { session, ordered: true });
      }
    });
  } finally {
    await session.endSession();
...
const randomAndEqualAssign = (
  ...
  for (const reviewerId of randomisedReviewers) {
    ...
    while (chosen.size < numReviewersNeeded) {
      candidates.sort((a, b) => (assignedForTeamMap.get(a)?.size ?? 0) - (assignedForTeamMap.get(b)?.size ?? 0));
      ...
      if (!assignedForTeamMap.has(next)) assignedForTeamMap.set(next, new Set());
