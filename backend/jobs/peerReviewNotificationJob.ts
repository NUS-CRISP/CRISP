...
const toObjectIdSet = (ids: Array<Types.ObjectId | string>) =>
	Array.from(new Set(ids.map(id => id.toString()))).map(id => new Types.ObjectId(id));
...
const mapTeamMembers = async (teamSetId: Types.ObjectId) => {
	const teams = await TeamModel.find({ teamSet: teamSetId }).select('_id members');
...
const countRemainingReviewsByUser = (
	...
	for (const s of submissions) {
		...
		if ((s.reviewerKind === 'Student' || s.reviewerKind === 'TA') && s.reviewerUserId) {
...
const sendLifecycleNotifications = async (
	...
	if (isWithinFirstDayAfter(now, peerReview.startDate)) {
		...
		for (const acc of [...facultyAccounts, ...studentAccounts]) {
			await dispatchDeduped('PR_STARTED', 'once', peerReview._id, acc, subject, text);
		}
	}
	...
	if (isWithinFirstDayAfter(now, peerReview.endDate)) {
		...
		for (const acc of [...facultyAccounts, ...studentAccounts]) {
			await dispatchDeduped('PR_ENDED', 'once', peerReview._id, acc, subject, text);
		}
	}

	if (peerReview.gradingStartDate && isWithinFirstDayAfter(now, peerReview.gradingStartDate)) {
	...
	if (peerReview.gradingEndDate) {
		...
		if (GRADING_END_REMINDER_DAYS.includes(daysToGradingEnd) && now < peerReview.gradingEndDate) {
			for (const acc of taAccounts) {
				const userId = acc.user.toString();
...
export const runPeerReviewNotificationCheck = async () => {
	...
	for (const pr of peerReviews) {
		const course = await CourseModel.findById(pr.course).select('code name faculty TAs');
		if (!course) continue;

		const { teamToMembers, studentUserIds } = await mapTeamMembers(pr.teamSetId);
		...
		const remainingByUser = countRemainingReviewsByUser(submissions, teamToMembers);
		...
		for (const submission of submissions) {
			if (submission.reviewerKind !== 'TA' || !submission.reviewerUserId) continue;
			if (submission.status === 'Submitted') continue;
			const userId = submission.reviewerUserId.toString();
			taRemainingToReview.set(userId, (taRemainingToReview.get(userId) ?? 0) + 1);
		}
		...
		for (const task of gradingTasks) {
			const graderId = task.grader.toString();
			taRemainingToGrade.set(graderId, (taRemainingToGrade.get(graderId) ?? 0) + 1);
		}

		const totalRemainingReviews = submissions.filter(s => s.status !== 'Submitted').length;
...
export const setupPeerReviewNotificationJob = () => {
	...
	cron.schedule('15 * * * *', async () => {
		console.log('Hourly peer review notification check:', new Date().toString());
		try {
