...
export const getPeerReviewAssignmentsByPeerReview = async (req: Request, res: Response) => {
  ...
  try {
    const peerReviewAssignments = await getPeerReviewAssignmentsByPeerReviewId(userId, userCourseRole, peerReviewId);
    ...
    if (error instanceof NotFoundError) {
      ...
      res.status(500).json({ message: 'Failed to get peer review assignments' });
    }
  }
}

export const getPeerReviewAssignmentsByTeam = async (req: Request, res: Response) => {
  const accountId = await getAccountId(req);
  const account = await AccountModel.findById(accountId);
  if (!account) {
    throw new MissingAuthorizationError('Access denied');
  ...
  try {
    const peerReviewAssignments = await getPeerReviewAssignmentsByTeamId(userCourseRole, teamId);
    res.status(200).json(peerReviewAssignments);
  } catch (error) {
    if (error instanceof NotFoundError) {
      res.status(404).json({ message: error.message });
      ...
      res.status(500).json({ message: 'Failed to get peer review assignments for team' });
    }
  }
}
...
export const randomAssignPeerReviews = async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(200).json({ message: 'Peer reviews randomly assigned successfully' });
  } catch (error) {
    if (error instanceof NotFoundError) {
      res.status(404).json({ message: error.message });
    } else {
      console.error('Error randomly assigning peer reviews:', error);
      res.status(500).json({ message: 'Failed to randomly assign peer reviews' });
    }
  }
}
