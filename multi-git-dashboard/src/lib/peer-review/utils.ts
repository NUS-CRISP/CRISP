...
export const fetchGithubRepoStructure = async (repoUrl: string): Promise<RepoNode> => {
...
export const fetchFileContent = async (repoUrl: string, filePath: string): Promise<string> => {
  const { owner, repo } = parseGithubRepo(repoUrl);
