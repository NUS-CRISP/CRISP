...
const PRSunburstGraph: React.FC<PRSunburstGraphProps> = ({ graphData }) => {
  ...
  useEffect(() => {
    ...
    graphData.edges.forEach(edge => {
      ...
      personCounts.set(sourceId, (personCounts.get(sourceId) || 0) + edge.weight);
    });
    ...
    const paths = g.selectAll("path")
      ...
      .join("path")
      .attr("fill", d => {
        if (d.depth === 1) return d3.schemeCategory10[d.data.name.charCodeAt(0) % 10];
    ...
    // Add labels
    g.selectAll("text")
      .data(root.descendants().filter(d => (d.y0 + d.y1) / 2 * Math.PI < 4 && d.depth > 0))
      .join("text")
      .attr("transform", d => {
        const x = (d.x0 + d.x1) / 2;
        const y = Math.sqrt((d.y0 + d.y1) / 2);
        const rotate = x < Math.PI ? x * 180 / Math.PI - 90 : x * 180 / Math.PI + 90;
      ...
      if (sourcePos && targetPos) {
        ...
        if (distance > 10) {
          ...
          connections.append("path")
            .attr("d", `M${sourcePos.x},${sourcePos.y} Q${curveX},${curveY} ${targetPos.x},${targetPos.y}`)
            .attr("fill", "none")
            .attr("stroke", colorMap[edge.status as keyof typeof colorMap] || "#999")
            .attr("stroke-width", Math.max(1, Math.min(edge.weight * 0.5, 3)))
            .attr("opacity", 0.7)
            .attr("marker-end", `url(#arrow-${edge.status})`)
            .on("mouseover", function(event) {
              d3.select(this).attr("opacity", 1).attr("stroke-width", Math.max(2, Math.min(edge.weight * 0.8, 4)));
  ...
  return (
    <div style={{ position: "relative", width: "700px", height: "600px" }}>
      ...
      ></div>
      <svg ref={svgRef} width="700" height="600" style={{ border: "1px solid #ccc", borderRadius: "8px" }}></svg>
    </div>
  );
};
