...
program
  ...
  .option('-o, --output <directory>', 'Output directory for PNG files', './pr-exports')
  .option('-t, --type <type>', 'Diagram type to export (arc, network, all)', 'all')
  ...
  .option('-s, --selector <selector>', 'Custom selector for the diagram SVG', '')
...
async function exportDiagrams() {
  ...
  if (options.interactive) {
    ...
    config.type = await prompt('Enter diagram type to export (arc, network, all)', config.type);
    config.delay = await prompt('Enter delay between actions in ms', config.delay);
    
    const customSelector = await prompt('Enter custom SVG selector (leave empty for default)', '');
    if (customSelector) {
