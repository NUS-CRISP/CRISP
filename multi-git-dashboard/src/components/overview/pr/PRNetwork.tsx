...
const PRGraph: React.FC<PRGraphProps> = ({ graphData }) => {
    ...
    useEffect(() => {
        ...
        const simulation = d3
            ...
            .force("link", d3.forceLink(filteredData.edges).id((d: any) => d.id).distance(300))
        ...
        const link = svg
            ...
            .enter()
            .append("path")
            .attr("stroke", (d) => colorMap[d.status as keyof typeof colorMap] || colorMap.default)
        ...
        simulation.on("tick", () => {
            link.attr("d", (d: any) => {
                ...
                const statusDirection = d.status === "approved" ? 1 : 
                                 d.status === "changes_requested" ? -1 : 
                                 d.status === "commented" ? 0.5 : 0;
