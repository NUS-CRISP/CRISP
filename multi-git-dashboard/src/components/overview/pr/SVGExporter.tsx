...
import { Button, Box, Group, Stack, Text, Progress, Modal, Tabs, Switch, Select } from '@mantine/core';
...
const SVGExporter = ({ 
    ...
    for (const type of typesToExport) {
      for (let i = 0; i < weekRangesToExport.length; i++) {
        ...
        if (svgElement) {
          ...
          console.error(`SVG element not found for ${type} diagram, week range ${rangeLabel}`);
        }
  ...
  return (
    <Stack spacing="md">
      <Box>
        <Text size="xl" weight={700}>SVG Diagram Exporter</Text>
        <Text size="sm" color="dimmed">Export week-by-week PR diagrams as PNG files</Text>
      </Box>
      ...
      <Tabs defaultValue="settings">
          ...
          <Stack>
            <Select
              ...
              data={[
                ...
                { value: EXPORT_TYPES.NETWORK_DIAGRAM, label: 'Network Diagram' }
              ]}
            ...
            <Button 
              ...
              fullWidth
            >
              {exporting ? 'Exporting...' : 'Export Diagrams for All Week Ranges'}
            </Button>
          </Stack>
        </Tabs.Panel>
        ...
        <Tabs.Panel value="preview" pt="md">
          <Box>
            <Text weight={500} mb={10}>Week Ranges to Export:</Text>
            <Stack spacing={0}>
      ...
      {exporting && (
        <Box>
          <Text size="sm" mb={5}>
            Exporting {currentRange}... ({exportedCount}/{weekRangesToExport.length * (exportAllTypes ? Object.keys(EXPORT_TYPES).length : 1)})
          </Text>
