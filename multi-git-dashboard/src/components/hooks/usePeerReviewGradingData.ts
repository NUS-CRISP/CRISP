...
export default function usePeerReviewGradingData({
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      const tree = await fetchGithubRepoStructure(repoUrl, res.assignment?.repo?.commitOrTag);
      setRepoTree(tree);

      const files = flattenTree(tree);
      if (files[0]) {
        setCurrFile(files[0]);
        const content = await fetchFileContent(repoUrl, files[0], res.assignment?.repo?.commitOrTag);
  ...
  const openFile = useCallback(
    async (filePath: string) => {
      ...
      if (!fileContent[filePath]) {
        const content = await fetchFileContent(repoUrl, filePath, dto?.assignment?.repo?.commitOrTag);
